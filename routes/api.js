...
router.post('/sensit/data', function(req, res, next) {
    ...
    pool.query('insert into data (temp,hum,light) values(?,?,?)', [String(req.body.temperature).substring(0,3),String(req.body.humidity).substring(0,3),light], function (err, rows) {
        if(!err){
            res.sendStatus(200);
        }else{
            res.sendStatus(500);
